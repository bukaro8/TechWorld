					...
					<div className=' flex  flex-col m-8 w-72 max-w-sm bg-white rounded-lg shadow-md dark:bg-slate-300 dark:border-gray-700 hover:bg-blue-200 '>
						<Link to={`/product/${id}`}>
							<picture className='h-64  rounded flex items-center justify-center '>
								<img
									className='p-8  rounded-t-lg m max-h-full '
									src={image}
									alt={image}
								/>
							</picture>
						</Link>
						<div className='rounded border-red-800 px-2'>
							<Link to={`/product/${id}`}>
								{name.length > 60 ? (
									<h5 className=' text-center text-lg  font-semibold tracking-tight text-gray-900 dark:text-white h-20'>
										{name.slice(0, 57) + '...'}
									</h5>
								) : (
									<h5 className=' text-center text-lg  font-semibold tracking-tight text-gray-900 dark:text-white h-20'>
