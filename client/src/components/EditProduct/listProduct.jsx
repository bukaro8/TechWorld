...
export default function List() {
	...
	return (
		<div>
			...
			<div className='grid grid-rows-1 gap-y-5 gap-x-4 sm:grid-rows-2 lg:grid-rows-3 xl:grid-rows-4 xl:gap-x-4 '>
				<div className='grid grid-cols-5 justify-items-center content-center '>
					<div> </div>
					<div className='text-lg p-0 font-semibold text-violet-500'>
						Nombre
					</div>
					<div className='text-lg p-0 font-semibold text-violet-500'>
						Precio
				...
				{error ? (
					...
					dataProductox?.map((data) => (
						<ProductoInd key={data.id} data={data} />
					))
				)}
			</div>
		</div>
