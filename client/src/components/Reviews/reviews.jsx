	...
	return (
		<div className='max-w-2xl mx-auto px-4'>
			{isAuthenticated ? (
							...
							<div className='flex flex-wrap -mx-3 mb-6'>
								...
								{!comment.comment ? (
									<div className='-mr-1'>
										<button
											type='submit'
											className='bg-white text-gray-700 font-medium py-1 px-4 border border-gray-400 rounded-lg tracking-wide mr-1 hover:bg-gray-100 opacity-50 cursor-not-allowed'
										>
											Post comment
										</button>
									</div>
				...
				<div>"You must be logged to leave a comment" </div>
			)}
									...
									return (
										<div key={r.id} className='mt-4'>
											<div className='flex justify-between'>
												<p className='text-lg text-gray-700 font-semibold hover:underline cursor-pointer capitalize'>
													{r.name}
												</p>
												<div className='flex  w-1/5 justify-between items-center'>
													<p>{r.rating}</p>
													<div className='flex'>
														{starCreator(r.rating).map((el) => (
